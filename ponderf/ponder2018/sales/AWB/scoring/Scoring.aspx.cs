...
using ponder2018;
    ...
    public partial class sales_AWB_scoring_Scoring : System.Web.UI.Page
    {
    ...
    private string GetAccessGroups(string attUid, string baseApiUrl, string authToken, ref string accessControlId)
    {
        try
        {
            ...
            webRequest = WebRequest.Create(baseApiUrl + "/users/AWB_SCORING");
    ...
    private void RegisterModulesScriptTag()
        {
            ...
            string[] fileEntries = Directory.GetFiles(AppDomain.CurrentDomain.BaseDirectory + $"/sales/AWB/scoring/{scriptDirectory}");
            for (int i = 0; i < fileEntries.Length; i++)
            {

                if (Path.GetFileName(fileEntries[i]).IndexOf("main.") != -1 && Path.GetExtension(fileEntries[i]) == ".js")
                {
                    ltlModulesJsTag.Text = String.Format("<script type='text/javascript' src='/sales/AWB/scoring/{0}/{1}'></script>", scriptDirectory, Path.GetFileName(fileEntries[i]));
                }
            }

            ltlHeaderLinks.Text = String.Format(@"<link rel='stylesheet' type='text/css' href='/sales/AWB/scoring/{0}/leaflet.css'/>
                                                  <script src='/sales/AWB/scoring/{1}/jquery-3.6.0.min.js'></script>
                                                   <script src='/sales/AWB/scoring/{2}/leaflet.js'></script>
                                                    <script src='/sales/AWB/scoring/{3}/ramda.min.js'></script>
                                                    <link rel='stylesheet' type='text/css' href='/sales/AWB/scoring/{4}/semantic.min.css' />
                                                    <link rel='stylesheet' type='text/css' href='/sales/AWB/scoring/{5}/style.css' />
                                                    <script src='/sales/AWB/scoring/{6}/semantic.min.js'></script>
                                                    <link rel='stylesheet' type='text/css' href='/sales/AWB/scoring/{7}/tabulator.min.css' />
                                                    <link rel='stylesheet' type='text/css' href='/sales/AWB/scoring/{8}/tabulator_semantic-ui.min.css' />
                                                   ",
    ...
    [WebMethod]
        public static void LogAcknowledge()
        {
            ...
            logEventParameters.eventType = "AWB Wireless Scoring Tool";
