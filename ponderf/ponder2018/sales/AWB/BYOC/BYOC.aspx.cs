...
public partial class sales_AWB_BYOC_BYOC : System.Web.UI.Page
{
    private string GetAccessGroups(string attUid, string baseApiUrl, string authToken, ref string accessControlId)
    {
        try
        {
            ...
            webRequest = WebRequest.Create(baseApiUrl + "/users/AWB_BYOC");
    ...
    private void RegisterModulesScriptTag()
    {
        ...
        string[] fileEntries = Directory.GetFiles(AppDomain.CurrentDomain.BaseDirectory + $"/sales/AWB/BYOC/{scriptDirectory}");
        for (int i = 0; i < fileEntries.Length; i++)
        {

            if (Path.GetFileName(fileEntries[i]).IndexOf("main.") != -1 && Path.GetExtension(fileEntries[i]) == ".js")
            {
                ltlModulesJsTag.Text = String.Format("<script type='text/javascript' src='/sales/AWB/BYOC/{0}/{1}'></script>", scriptDirectory, Path.GetFileName(fileEntries[i]));

            }
        }

        ltlHeaderLinks.Text = String.Format(@"<link rel='stylesheet' type='text/css' href='/sales/AWB/BYOC/{0}/leaflet.css'/>
                                        <script src='/sales/AWB/BYOC/{1}/jquery-3.6.0.min.js'></script>
                                        <script src='/sales/AWB/BYOC/{2}/leaflet.js'></script>
                                        <script src='/sales/AWB/BYOC/{3}/ramda.min.js'></script>
                                        <link rel='stylesheet' type='text/css' href='/sales/AWB/BYOC/{4}/semantic.min.css' />
                                        <link rel='stylesheet' type='text/css' href='/sales/AWB/BYOC/{5}/style.css' />
                                        <script src='/sales/AWB/BYOC/{6}/semantic.min.js'></script>
                                        ",
    ...
    private static List<Dictionary<string, object>> SerializeDataTable(DataTable dataTable)
    {
        ...
        {
            int clusterId = 0;
            string insertClusterSql = @"INSERT INTO PONDER.SALES_AWB_BYOC_LIST (BYOC_CLUSTER_NAME, ADMIN_USER, VIEW_USER,
            ...
            string insertClusterRecordSql = @"INSERT INTO PONDER.SALES_AWB_BYOC_DETAIL
        ...
        public static List<Dictionary<string, object>> Clusters()
        {
            ...
            string sql = string.Format(@"select PONDER.SALES_AWB_BYOC_LIST.*, NUMBER_OF_LOCATIONS,
                                        TO_CHAR(PONDER.SALES_AWB_BYOC_LIST.CREATE_DATE, 'MON-DD-YYYY') AS CLUSTER_CREATE_DATE,
                                        TO_CHAR(PONDER.SALES_AWB_BYOC_LIST.LAST_RUN_DATE, 'MON-DD-YYYY') AS CLUSTER_LAST_RUN_DATE
                                        FROM PONDER.SALES_AWB_BYOC_LIST INNER JOIN
                                        (SELECT COUNT(*) AS NUMBER_OF_LOCATIONS, BYOC_CLUSTER_ID FROM PONDER.SALES_AWB_BYOC_DETAIL GROUP BY BYOC_CLUSTER_ID) A
                                        ON PONDER.SALES_AWB_BYOC_LIST.BYOC_CLUSTER_ID = A.BYOC_CLUSTER_ID 
                                        WHERE IS_ACTIVE=1 AND (ADMIN_USER='{0}' OR  VIEW_PUBLIC=1 OR (ADMIN_USER <> '{1}' AND INSTR(VIEW_USER, '{2}') <> 0))
                                        ORDER BY SALES_AWB_BYOC_LIST.BYOC_CLUSTER_ID DESC", ATTUID, ATTUID, ATTUID);
        ...
        public static List<Dictionary<string, object>> RerunCluster(int clusterId)
        {
            ...
            string selectOldClusterSql = string.Format(@"SELECT BYOC_CLUSTER_NAME, ADMIN_USER, VIEW_USER,VIEW_PUBLIC FROM PONDER.SALES_AWB_BYOC_LIST
                                        WHERE BYOC_CLUSTER_ID={0}", clusterId);
            string insertNewClusterSql = @"INSERT INTO PONDER.SALES_AWB_BYOC_LIST (BYOC_CLUSTER_NAME, ADMIN_USER, VIEW_USER,VIEW_PUBLIC, CREATE_DATE, IS_ACTIVE, IS_PROCESSED, BATCH_ID)
                                        VALUES ( :byocClusterName, :adminUser, :viewUser,:viewPublic,SYSDATE,1,0,:oldClusterId )
                                        returning BYOC_CLUSTER_ID into :newClusterId";
            string deleteClusterSql = "UPDATE PONDER.SALES_AWB_BYOC_LIST SET IS_ACTIVE=0 WHERE BYOC_CLUSTER_ID = :oldClusterId";
            string updateClusterDetailsSql = "UPDATE PONDER.SALES_AWB_BYOC_DETAIL SET BYOC_CLUSTER_ID=:newClusterId WHERE BYOC_CLUSTER_ID=:oldClusterId";
        ...
        {
            string deleteClusterSql = "UPDATE PONDER.SALES_AWB_BYOC_LIST SET IS_ACTIVE=0 WHERE BYOC_CLUSTER_ID = :clusterId";
        ...
        public static List<Dictionary<string, object>> Cluster()
        {
            ...
            string getClusterSql = string.Format(@"select PONDER.SALES_AWB_BYOC_LIST.* FROM PONDER.SALES_AWB_BYOC_LIST WHERE BYOC_CLUSTER_ID = {0}", clusterId );
        ...
        public static List<Dictionary<string, object>> ClusterDetails()
        {
            ...
            string getClusterDetailSql = string.Format(@"select address, city, state, zipcode, latitude, longitude, location_id, company_name, notes,
                ...
                speed_experience,azimuth,usid FROM PONDER.SALES_AWB_BYOC_DETAIL WHERE BYOC_CLUSTER_ID = {0}", clusterId);
