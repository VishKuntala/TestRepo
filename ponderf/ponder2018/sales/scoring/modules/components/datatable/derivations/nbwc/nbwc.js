import { html } from '../../../../../templates/components/datatables/nbwcDatatable.js';
import $ from "jquery";
...
const nbwcTable = ({ adminUser, instance }) => {
    ...
    const fetchData = async () => {
        let { svIds, salesRepId, statecode, covScore, fiveg, band14, tputText, roamingDependency, zipcode, smId, sortCriteria, legacyPen } = instance.store;
        if (smId === '' && salesRepId === '') {
            return;
        }
    ...
    const initSubscriptions = () => {
        instance.stateManager.subscribe("renderTable",
            async d => {
                if (d) {
                    // const takeFiveHundred = R.take(500, d);
                    await instance.reloadTableData(instance.store.tableData);
                    /*if (instance.table) {
