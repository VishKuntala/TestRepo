...
class Filter extends PonderComponent {
    ...
    async fetchData() {
        ...
        // fetch initial map data (columns lat,lng, campaign_id and indoor_coverage_score without limits)        
        results.push($.ajax({
            url: `${instance.store.config.baseUrl}/MapData`,
            headers: { "Content-Type": "application/json" },
            type: 'post',
            data: JSON.stringify(Object.assign(postData, {attribute: (instance.store.config.map && instance.store.config.map.heatmapAttribute) || 'INDOOR_COVERAGE_SCORE', limit: null })),
            processData: false
        }));
                    ...
                    try {
                        ...
                        for (let i = 0; i < $(`ponder-filter ponder-search-dd`).length; i += 1) {
                            ...
                            if (filterDropdown.requeryOnSelect || (filterDropdown.allowMultipleSelection
                                ...
                                // replace commas in the value with caret to avoid wrong splits on the commas
                                const results = R.map(item => R.merge(item, { name: displayTextTransformer[filterDropdown.dataKey] ? displayTextTransformer[filterDropdown.dataKey][item.name] : item.name, value: !isNaN(item.value) ? item.value.toString() : item.value.replace(/,/g, '^') }), response.d);
    ...
    async firstUpdated(changedProperties) {
        ...
        if (this.derivation) {
            ...
            $('.filter.container').html(this.derivedClass.html({ id: this.id, config: this.store.config }));
        }
