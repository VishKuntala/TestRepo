...
public partial class sales_scoring_GlobalBusiness : System.Web.UI.Page
{
        ...
        string statecode, string zipcode, string fiveg, string band14, string speedExperience, string availCapacity, string roamingDependency, string indoorCovScore, string fiberStatus)
    {
             ...
             string valueColumn, string nameColumn, string sortColumn, string sortDirection = "ASC")
        {
            ...
            {
                sql = string.Format("select distinct {0} as value, {1} as name from PONDER.SALES_GLOBAL_PROD WHERE 1=1 AND ", valueColumn, nameColumn);
                sql = sql + BuildWhereClause(industry, svIds, companyNames, statecode,zipcode, fiveg, band14, speedExperience, availCapacity, roamingDependency, indoorCovScore, fiberStatus);
            }
            else
            {
                sql = string.Format("select distinct {0} as value, {1} as name from PONDER.SALES_GLOBAL_PROD WHERE LOWER({2}) LIKE '{3}%' AND ", valueColumn, nameColumn, nameColumn.ToLower(), searchString.ToLower());
                sql = sql + BuildWhereClause(industry, svIds, companyNames, statecode,zipcode, fiveg, band14, speedExperience, availCapacity, roamingDependency, indoorCovScore, fiberStatus);
            }
        ...
        {
            string sql = "SELECT ROWNUM, A.* FROM (SELECT coverage_score_rank,svid, INDUSTRY, COMPANY_NAME, indoor_coverage_score, latitude, longitude, " +
            ...
            sql = sql + " FROM PONDER.SALES_GLOBAL_PROD WHERE INDOOR_COVERAGE_SCORE IS NOT NULL AND COVERAGE_SCORE_RANK != 0 AND ";
            sql = sql + BuildWhereClause(industry, svIds, companyNames, statecode, zipcode, fiveg, band14, speedExperience, availCapacity, roamingDependency, indoorCovScore, fiberStatus);
        ...
        string sortCriteria, string limit, string attribute)
    {
        string sql = "SELECT ROWNUM, A.* FROM (SELECT latitude, longitude, " + attribute +
            " FROM PONDER.SALES_GLOBAL_PROD WHERE INDOOR_COVERAGE_SCORE IS NOT NULL AND COVERAGE_SCORE_RANK != 0 AND ";
        sql = sql + BuildWhereClause(industry, svIds, companyNames, statecode, zipcode, fiveg, band14, speedExperience, availCapacity, roamingDependency, indoorCovScore, fiberStatus);
    ...
    {
        string sql = @"SELECT COVERAGE_SCORE_RANK, SVID, INDUSTRY, COMPANY_NAME, INDOOR_COVERAGE_SCORE, latitude, longitude, " +
            "FIVEG, BAND_14, FIRST_NET, ROAM_TEXT, FIVEG_PLUS_MMWAVE, SPEED_EXPERIENCE, AVAIL_CAPACITY, FIBER_STATUS, " +
            "ADDR,CITY,STATE_Y,ZIPCODE FROM PONDER.SALES_GLOBAL_PROD WHERE LATITUDE = '" + latitude + "' AND LONGITUDE = '" + longitude + "'";
        DataTable dataTable = DatabaseHelper.GetDataTable(sql);
        return SerializeDataTable(dataTable);
    }
