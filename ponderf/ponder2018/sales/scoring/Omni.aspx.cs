...
using ponder2018;
    ...
    public partial class sales_scoring_Omni : System.Web.UI.Page
    {
             ...
             string valueColumn, string nameColumn, string sortColumn, string sortDirection = "ASC")
        {
            ...
            if (string.IsNullOrEmpty(searchString))
            {
                ...
                sql = sql + BuildWhereClause(smId, salesRepId, svIds, zipcode, statecode, indoorCovScore, fiveg, band14, tputText, roamingDependency, legacyPen);
            }
            else
            {
        ...
        {
            string sql = "SELECT ROWNUM, A.* FROM (SELECT coverage_score_rank,svid, svid_name,indoor_coverage_score, latitude, longitude, " +
            ...
            sql = sql +    " FROM PONDER.SALES_OMNI_V2 WHERE INDOOR_COVERAGE_SCORE IS NOT NULL AND COVERAGE_SCORE_RANK != 0 AND ";
            sql = sql + BuildWhereClause( smId,  salesRepId,  svIds, zipcode,  statecode, indoorCovScore,  fiveg,  band14, tputText,  roamingDependency, legacyPen);
...
string sortCriteria, string limit, string legacyPen, string attribute)
        {
            string sql = "SELECT ROWNUM, A.* FROM (SELECT latitude, longitude, indoor_coverage_score, " + attribute +
                " FROM PONDER.SALES_OMNI_V2 WHERE INDOOR_COVERAGE_SCORE IS NOT NULL AND COVERAGE_SCORE_RANK != 0 AND ";
            sql = sql + BuildWhereClause(smId, salesRepId, svIds, zipcode, statecode, indoorCovScore, fiveg, band14, tputText, roamingDependency, legacyPen);
...
string sortCriteria, string limit, string legacyPen, string attribute)
        ...
        {
            string sql = @"SELECT coverage_score_rank,svid, svid_name,indoor_coverage_score, latitude, longitude, " +
                "FIVEG, BAND_14, FIRST_NET, ROAM_TEXT, SM_ATTUID, OWNING_ATTUID,FIVEG_PLUS,TPUT_TEXT,FIBER_CUST," +
