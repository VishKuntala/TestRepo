...
class Filter extends PonderComponent {
    ...
    async zoomToSalesMarket() {
        ...
        if (instance.store.config.filter.zoomToAreaDataKey && instance.canZoomToSalesMarket) {
            const boundaryData = await instance.getData(
                `${instance.store.baseSearchApiUrl}/geodata/none/salesMarketBoundary.geojson?id=${instance.store[instance.store.config.filter.zoomToAreaDataKey]}`
            );
            const salesMarketCenter = centroid(boundaryData.data);
    ...
    async zoomToArea() {
        const instance = this;
        const postData = buildPayloadFromStore(instance);
        const boundaryResponse = await instance.postData(`${instance.store.baseSearchApiUrl}/wst-geodata-json/areaBoundary.geojson`, postData);
        if (boundaryResponse.data.features[0]?.geometry?.type === 'Point') {
            instance.store.zoomToPoint = {
                lat: boundaryResponse.data.features[0].geometry.coordinates[1],
                lng: boundaryResponse.data.features[0].geometry.coordinates[0],
            };
        } else {
            instance.store.zoomToArea = boundaryResponse.data;
        }
    }

    async fetchData() {
        const instance = this;
        $("#addlInfo").html(`${instance.store.config.addlInfoLabel || 'Total Wireline Opportunities (as filtered)'}: <div class="ui active inline slow primary tiny loader"></div>`);
        $("#addlInfo1").html(`Total ${instance.store.config.tabs?.[0]?.label || 'Building Locations'} (as filtered): <div class="ui active inline slow primary tiny loader"></div>`);
        $("#addlInfo2").html(`Total ${instance.store.config.tabs?.[1]?.label || 'Business Tenants'} (as filtered): <div class="ui active inline slow primary tiny loader"></div>`);
        $("#mainTableTab").html(`${instance.store.config.tabs?.[0]?.label || 'Building Locations'}`); 
    ...
    async attachEventHandlers() {
        ...
        $(`#${instance.id} .ui.btnSubmit`).off().on('click', async function () {
            ...
            // const tenantTable = Tabulator.findTable('tenantTable');
            // tenantTable.clearData();
            // $(`.btnClear`).addClass('disabled');
            await instance.fetchData();
           // instance.store.isChannelActive = true;
           // instance.store.clearClusterLayer = false;
            if (instance.canZoomToArea) {
                await instance.zoomToArea();
                instance.canZoomToArea = false;
            }
           if (instance.store.config.attrPointLayerVisible) {
                instance.store.storeLayerVisibilityChange = { layerId: "attr-storepoint-layer-id", layerVisibilityStatus: true };
                $('#attr-storepoint-layer-id').prop('checked', true);
            } /* else {
                instance.store.storeLayerVisibilityChange = { layerId: "attr-storepoint-layer-id", layerVisibilityStatus: false };
        ...
        $(`.showFilter.button`).off().on('click', async function () {
            if ($(".filter.form").is(":visible")) {
                $(".showFilter.button").html('Show Filter');
                if (instance.store.pillsData.length) {
                    $(".saveFilter.button").show();
                } else {
        ...
        this.stateManager.subscribe("dropdownValueChanged",
            async d => {
                if (d && !R.is(Array, d)) {
                    ...
                    if (!instance.canZoomToArea) {
                        instance.canZoomToArea = instance.store.config.filter.mapResetDataKeys?.includes(dropdownInstance.dataKey);
                    }
    ...
    async firstUpdated(changedProperties) {
        ...
        this.store.mapLayersReady = this.store.config.loadDataOnChannelInit;
