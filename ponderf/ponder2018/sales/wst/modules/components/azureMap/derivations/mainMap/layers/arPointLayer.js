...
const arPointLayer = ({instance, beforeLayerId }) => {
    ...
    map.imageSprite.createFromTemplate('arIcon', 'ellipse-outline', color).then(function () {

        instance.getData(
            `${store.baseApiUrl}/store/locations?category=AR`,               
        ).then(function(res) {
            const src = new atlas.source.DataSource(source);
            // src.importDataFromUrl(`${store.baseApiUrl}/store/locations?category=AR`);
            src.add(res.data);
            map.sources.add(src);
