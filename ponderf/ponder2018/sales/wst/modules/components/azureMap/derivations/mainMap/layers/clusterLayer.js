...
const clusterLayer = ({ instance, beforeLayerId }, geojson, geoEntity) => {
    ...
    map.imageSprite.add('psa-icon', `${store.rootUrl}sales/wst/${imgUrl}`).then(function () {
        ...
        if (map.layers.getLayerById(`${id}-labels`)) {
            map.layers.remove(`${id}-labels`);
            instance.store.layersAddedToMap = instance.store.layersAddedToMap.filter (layerId => layerId !== `${id}-labels`);
        }
        ...
        const src = new atlas.source.VectorTileSource(source, {
            "tileSize": 256,
            "tiles": [`${store.baseApiUrl}/clusters/${store.config.tool}/location/${geoEntity || store.geoEntity}/{z}/{x}/{y}.mvt?${queryStringParams}`]
        });
        map.sources.add(src);
        ...
        if (store.searchMode !== 'businessSearch') { 
            ...
            const layer = new atlas.layer.BubbleLayer(src, id, {
                ...
                radius: 12,
                color: "#feb24c",
                opacity: ['step', ['zoom'], 0.6, 12.1, 0],
                strokeWidth: 0
            });

            const labelsLayer = new atlas.layer.SymbolLayer(src, `${id}-labels`, {
                sourceLayer: sourceLayer,
                visible: true,
                minZoom,
                maxZoom,
            ...
            map.layers.add([layer, labelsLayer]);
            instance.store.layersAddedToMap.push(id);
            instance.store.layersAddedToMap.push(`${id}-labels`);

        } else {
            const labelsLayer = new atlas.layer.SymbolLayer(src, id, {
                sourceLayer: sourceLayer,
                visible: true,
                minZoom,
                maxZoom,
            ...
            map.layers.add([layer, labelsLayer], 'labels');
            instance.store.layersAddedToMap.push(id);
        }
