...
const svidBarchart = ({ adminUser, instance }) => {
    ...
    const transformData = item => {
        ...
        const customerCategoryMapper = {
            "1": "Fiber Customer and Wireless Customer",
            "2": "Fiber Customer and Wireless Cross-Sell Opportunity",
            "3": "Wireless Customer and Fiber Cross-Sell Opportunity",
            "4": "Opportunity for Fiber and Wireless",
            "5": "Fiber Customer",
            "6": "Fiber Opportunity",
            "7": "MDU All AT&T Fiber Customers",
            "8": "MDU with AT&T Fiber Prospects"
        };
        ...
        if (R.has('indoor_coverage_score', item)) {
            item['indoor_coverage_score'] = covScoreMapper[item['indoor_coverage_score']];
        }
        ...
        if (R.has('addr_customer_category_id', item)) {
            item['addr_customer_category_id'] = customerCategoryMapper[item['addr_customer_category_id']];
        }
        if (R.has('csi_eligible', item)) {
            item['csi_eligible'] = yesNoMapper[item['csi_eligible']];
        }
        if (R.has('hypergig', item)) {
            item['hypergig'] = yesNoMapper[item['hypergig']];
        }
