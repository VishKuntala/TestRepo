...
class SearchDropdown extends PonderComponent {
            ...
            for (let i = 0; i < keys.length; i += 1) {
                ...
                if (this.dataKey === key) {
                    if (instance.pillsLabel) {
                        ...
                        let pillsData = R.clone(instance.store.pillsData) || [];
                        const itemToAdd = `${instance.pillsLabel}:${this.store.config.filter.initialValues[key]}:${instance.id}:${instance.pillsPriority}:${instance.extDataUrl}`;
    ...
    // Decode the text to remove &amp; from pill box

    htmlDecode(encodedText) {
        const htmlDoc = new DOMParser().parseFromString(encodedText, "text/html");
        ...
        let dropdownConfig = {
            ...
            apiSettings: this.remoteUrl ? {
                ...
                onResponse: function (response) {
                    ...
                    if (instance.store.config.filter.dataFilter && instance.store.config.filter.dataFilter[instance.dataKey]) {
                        results = instance.store.config.filter.dataFilter[instance.dataKey](results);
                    }
                    return {
            ...
            onAdd: (addedValue, addedText, $addedChoice) => {
                ...
                if (instance.pillsLabel) {
                    // pillsData is an array of strings - each pill string is of the format <<pill title>>:<<pill value>>:<<id of the control that generated the pill>>:<<pillpriority>>:<<extDataUrl>>
                    let pillsData = R.clone(instance.store.pillsData) || [];
                    const itemToAdd = `${instance.pillsLabel}:${instance.pillsValueSource === "text" ? instance.htmlDecode(addedText) : instance.htmlDecode(addedValue)}:${instance.id}:${instance.pillsPriority}:${instance.extDataUrl}`;
                    pillsData = R.append(itemToAdd, pillsData);
                    instance.store.pillsData = pillsData;
                }
            },
            onRemove: (removedValue, removedText, $removedChoice) => {
                if (instance.pillsLabel) {
                    // pillsData is an array of strings - each pill string is of the format <<pill title>>:<<pill value>>:<<id of the control that generated the pill>>:<<pillpriority>>:<<extDataUrl>>
                    let pillsData = R.clone(instance.store.pillsData) || [];
                    const itemToRemove = `${instance.pillsLabel}:${instance.pillsValueSource === "text" ? instance.htmlDecode(removedText) : instance.htmlDecode(removedValue)}:${instance.id}:${instance.pillsPriority}:${instance.extDataUrl}`;
                    if (R.includes(`undefined`, itemToRemove)) {
                        pillsData = R.without(R.filter(item => R.contains(`:${instance.id}`, item), pillsData), pillsData);
                    } else {
            ...
            onChange: (value, text, $choice) => {
                ...
                if (!instance.allowMultipleSelection && instance.pillsLabel) {
                    ...
                    if (text) { // item has been selected
                        const itemToAdd = `${instance.pillsLabel}:${instance.pillsValueSource === "text" ? instance.htmlDecode(text) : instance.htmlDecode(value)}:${instance.id}:${instance.pillsPriority}:${instance.extDataUrl}`;
                        pillsData = R.filter(item => !R.contains(`:${instance.id}`, item), pillsData);
                        pillsData = R.append(itemToAdd, pillsData);
