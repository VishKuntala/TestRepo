...
const fibUpgradeAddressLayer = ({ instance, beforeLayerId }) => {
    ...
    const {
        ...
        imgUrl,        
        minShapeSize,
    } = pointsLayers.ATT_FIBER_UPGRADES;

    map.loadImage(`${store.rootUrl}sales/opportunity/${imgUrl}`, function (error, image) {
        if (error) throw error;

        map.addImage("fibUpgradeAddressIcon", image, { sdf: true });
        ...
        map.addLayer(
            {
                ...
                layout: {
                    visibility: visibility,
                    "icon-image": "fibUpgradeAddressIcon",
            ...
            popup.setHTML(
                `
                  <div style="text-align: center">
                   ...
                   <br/><div style = "align:center"></div><a href="#" class="fibUpgradeAddressLayerPSAFilterLink">Click To Show Fiber Addresses within PSA ${popUpData.zip_co_psa}</a>
                  </div>
                `
            );
            $('.fibUpgradeAddressLayerPSAFilterLink').click(function () {
...
export default fibUpgradeAddressLayer;
