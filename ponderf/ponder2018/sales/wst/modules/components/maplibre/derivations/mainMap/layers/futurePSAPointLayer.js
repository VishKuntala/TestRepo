...
const futurePSAPointLayer = ({ instance, beforeLayerId }) => {
    ...
    const markerClicked = async event => {
        ...
        try {
            const { data } = await instance.getData(
                `${store.baseApiUrl}/point/futurepsas/${properties.zip_co_psa}`,
            );
            ...
            popup.setHTML(
                `
                  <div style="text-align: center">
                   ...
                   <br/><b>Future (T-30) Fiber Opportunity:</b> ${popUpData.future_fiber_opportunity}
                   <br/><b>T-30 File Date:</b> ${popUpData.grn_30day_dt}
                  </div>
                `
            );
        } catch (err) {
