...
const midmarketMap = ({ adminUser, instance }) => {
    ...
    const createMidMarketLayer = async title => {
        let pane = instance.map.getPane(title);
        if (!pane) {
            pane = instance.map.createPane(title);
            pane.style.zIndex = 400;
    ...
    const createMapLayers = async () => {
        ...
        const midmarketLayer = (await createMidMarketLayer('Mid Market Locations')).canvasLayer;
