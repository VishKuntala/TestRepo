...
public partial class sales_scoring_GlobalBusiness : System.Web.UI.Page
{
    ...
    private static string BuildWhereClause(string industry, string svIds, string companyNames,
        string noLowAccounts, string statecode, string zipcode, string fiveg, string band14, string tputText, string roamingDependency, string indoorCovScore)
    {
        string sql = "1=1";
        ...
        if (!string.IsNullOrWhiteSpace(noLowAccounts))
        {
        ...
        {
            string industry = queryStringCollection["industry"];
            string svIds = queryStringCollection["svIds"];
            string companyNames = queryStringCollection["companyNames"];
            string noLowAccounts = queryStringCollection["noLowAccounts"];
            ...
            {
                sql = string.Format("select distinct {0} as value, {1} as name from PONDER.SALES_GLOBAL WHERE 1=1 AND ", valueColumn, nameColumn);
                sql = sql + BuildWhereClause(industry, svIds, companyNames, noLowAccounts, statecode,zipcode, fiveg, band14, tputText, roamingDependency, indoorCovScore);
            }
            else
            {
                sql = string.Format("select distinct {0} as value, {1} as name from PONDER.SALES_GLOBAL WHERE LOWER({2}) LIKE '{3}%' AND ", valueColumn, nameColumn, nameColumn.ToLower(), searchString.ToLower());
                sql = sql + BuildWhereClause(industry, svIds, companyNames, noLowAccounts, statecode,zipcode, fiveg, band14, tputText, roamingDependency, indoorCovScore);
            }
        ...
        [WebMethod]
        public static List<Dictionary<string, object>> TableData(string industry, string svIds, string companyNames,
            string noLowAccounts, string statecode, string zipcode, string fiveg, string band14, string tputText, string roamingDependency, string indoorCovScore,
            string sortCriteria, string limit, string sortDirection, bool buildSortColumns)
        {
            string sql = "SELECT ROWNUM, A.* FROM (SELECT coverage_score_rank,svid, INDUSTRY, COMPANY_NAME, NO_LOW_ACCOUNT, coverage_score, latitude, longitude, " +
            ...
            sql = sql + " FROM PONDER.SALES_GLOBAL WHERE COVERAGE_SCORE IS NOT NULL AND COVERAGE_SCORE_RANK != 0 AND ";
            sql = sql + BuildWhereClause(industry, svIds, companyNames, noLowAccounts, statecode, zipcode, fiveg, band14, tputText, roamingDependency, indoorCovScore);
    ...
    [WebMethod]
    public static List<Dictionary<string, object>> MapData(string industry, string svIds, string companyNames,
        string noLowAccounts, string statecode, string zipcode, string fiveg, string band14, string tputText, string roamingDependency, string indoorCovScore,
        string sortCriteria, string limit, string attribute)
    {
        string sql = "SELECT ROWNUM, A.* FROM (SELECT latitude, longitude, " + attribute +
            " FROM PONDER.SALES_GLOBAL WHERE COVERAGE_SCORE IS NOT NULL AND COVERAGE_SCORE_RANK != 0 AND ";
        sql = sql + BuildWhereClause(industry, svIds, companyNames, noLowAccounts, statecode, zipcode, fiveg, band14, tputText, roamingDependency, indoorCovScore);
    ...
    [WebMethod]
    public static List<Dictionary<string, object>> ChartData(string industry, string svIds, string companyNames,
        string noLowAccounts, string statecode, string zipcode, string fiveg, string band14, string tputText, string roamingDependency, string indoorCovScore,
        string sortCriteria, string limit, string attribute)
    {
    ...
    [WebMethod]
    public static List<Dictionary<string, object>> LocationData(string latitude, string longitude) // Map popup info box data
    {
        string sql = @"SELECT COVERAGE_SCORE_RANK, SVID, INDUSTRY, NO_LOW_ACCOUNT, COMPANY_NAME, COVERAGE_SCORE, latitude, longitude, " +
            "FIVEG, BAND_14, FIRST_NET, ROAM_TEXT, FIVEG_PLUS, TPUT_TEXT, FIBER_CUST," +
            "ADDR,CITY,STATE_Y,ZIPCODE FROM PONDER.SALES_GLOBAL WHERE LATITUDE = '" + latitude + "' AND LONGITUDE = '" + longitude + "'";
        DataTable dataTable = DatabaseHelper.GetDataTable(sql);
        return SerializeDataTable(dataTable);
    }
    ...
    public static List<Dictionary<string, object>> Industries()
    {
        return FetchData(HttpContext.Current.Request.QueryString, "INDUSTRY", "INDUSTRY", "INDUSTRY");
    }
    ...
    [WebMethod]
    [ScriptMethod(UseHttpGet = true)]
    public static List<Dictionary<string, object>> NoLowAccounts()
    {
        return FetchData(HttpContext.Current.Request.QueryString, "NO_LOW_ACCOUNT", "NO_LOW_ACCOUNT", "NO_LOW_ACCOUNT");
    }
    ...
    {
        return FetchData(HttpContext.Current.Request.QueryString, "SVID", "SVID", "SVID");
    }
