...
const copperUpgradeandWirelessCustomerLayer = ({ instance, beforeLayerId, pointMinShapeSize }) => {
    ...
    const {
        ...
        imgUrl,
        minShapeSize
    } = pointsLayers.COPPER_UPGRADE_AND_WIRELSS_CUSTOMER;
    ...
    //const iconPromises = [
    //    map.imageSprite.createFromTemplate(`copperUpgradeandWirelessCustomerIcon`, 'ponder-ellipse', color)
    //];
    ...
    //Promise.all(iconPromises).then(function () {
        ...
        const src = new atlas.source.VectorTileSource(source, {
            "tileSize": 256,
            "tiles": [`${store.baseApiUrl}/tiles/${store.heatmap}/addresses/{z}/{x}/{y}.mvt?customerCategoryId=3&searchedPSA=${store.searchedPSA}`]
        });
        map.sources.add(src);
        ...
        const layer = new atlas.layer.SymbolLayer(src, id, {
            ...
            minZoom,
            iconOptions: {
                image: 'copperUpgradeandWirelessCustomerIcon',
                allowOverlap: true,
                anchor: 'center',
        ...
        map.layers.add([layer], config.pointsLayers.COPPER_UPGRADE_AND_WIRELSS_CUSTOMER.id);
...
/*    });*/
    ...
    //    map.addImage("copperUpgradeandWirelessCustomerIcon", image, { sdf: true });
    ...
    //            layout: {
    //                visibility: visibility,
    //                "icon-image": "copperUpgradeandWirelessCustomerIcon",
...
export default copperUpgradeandWirelessCustomerLayer;
