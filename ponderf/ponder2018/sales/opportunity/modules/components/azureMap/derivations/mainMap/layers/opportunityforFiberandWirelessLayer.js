...
const opportunityforFiberandWirelessLayer = ({ instance, beforeLayerId, pointMinShapeSize }) => {
    ...
    //const iconPromises = [
    //    map.imageSprite.createFromTemplate(`opportunityforFiberandWirelessIcon`, 'ponder-ellipse', color)
    //];
    ...
    //Promise.all(iconPromises).then(function () {
        ...
        const src = new atlas.source.VectorTileSource(source, {
            "tileSize": 256,
            "tiles": [`${store.baseApiUrl}/tiles/${store.heatmap}/addresses/{z}/{x}/{y}.mvt?customerCategoryId=6&searchedPSA=${store.searchedPSA}`]
        });
        map.sources.add(src);
        ...
        const layer = new atlas.layer.SymbolLayer(src, id, {
            ...
            minZoom,
            iconOptions: {
                image: 'opportunityforFiberandWirelessIcon',
                allowOverlap: true,
                anchor: 'center',
        ...
        map.layers.add([layer], config.pointsLayers.OPPORTUNITY_FOR_FIBER_AND_WIRELESS.id);
