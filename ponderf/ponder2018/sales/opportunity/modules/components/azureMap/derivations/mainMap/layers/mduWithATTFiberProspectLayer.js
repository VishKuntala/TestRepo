...
const mduWithATTFiberProspectLayer = ({ instance, beforeLayerId, pointMinShapeSize }) => {
    ...
    //const iconPromises = [
    //    map.imageSprite.createFromTemplate(`mduWithATTFiberProspectIcon`, 'ponder-acc-rectangle', color)
    //];
    ...
    const src = new atlas.source.VectorTileSource(source, {
        "tileSize": 256,
        "tiles": [`${store.baseApiUrl}/tiles/${store.heatmap}/addresses/{z}/{x}/{y}.mvt?customerCategoryId=8&searchedPSA=${store.searchedPSA}`]
    });
    map.sources.add(src);
    ...
    const layer = new atlas.layer.SymbolLayer(src, id, {
        ...
        minZoom,
        iconOptions: {
            image: 'mduWithATTFiberProspectIcon',
            allowOverlap: true,
            anchor: 'center',            
            size
        }
    ...
    map.layers.add([mduOutlineLayer, layer], config.pointsLayers.MDU_WITH_ATT_FIBER_PROSPECTS.id);
    ...
    //        "type": "vector",
    //        "tiles": [`${store.baseApiUrl}/tiles/${store.heatmap}/addresses/{z}/{x}/{y}.mvt?customerCategoryId=8`],
    //        "tolerance": 0,
    //        promoteId,
    //    });
