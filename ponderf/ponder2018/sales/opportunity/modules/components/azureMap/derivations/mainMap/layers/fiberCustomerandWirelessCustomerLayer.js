...
const fiberCustomerandWirelessCustomerLayer = ({ instance, beforeLayerId, pointMinShapeSize }) => {
    ...
    const {
        ...
        imgUrl,
        minShapeSize
    } = pointsLayers.FIBER_CUSTOMER_AND_WIRELESS_CUSTOMER;
    ...
    //const iconPromises = [
    //    map.imageSprite.createFromTemplate(`fiberCustomerandWirelessCustomerIcon`, 'ponder-ellipse', color)
    //];
    ...
    //Promise.all(iconPromises).then(function () {
        ...
        const src = new atlas.source.VectorTileSource(source, {
            "tileSize": 256,
            "tiles": [`${store.baseApiUrl}/tiles/${store.heatmap}/addresses/{z}/{x}/{y}.mvt?customerCategoryId=1&searchedPSA=${store.searchedPSA}`]
        });
        map.sources.add(src);
        ...
        const layer = new atlas.layer.SymbolLayer(src, id, {
            ...
            minZoom,
            iconOptions: {
                image: 'fiberCustomerandWirelessCustomerIcon',
                allowOverlap: true,
                anchor: 'center',
        ...
        map.layers.add([layer], config.pointsLayers.FIBER_CUSTOMER_AND_WIRELESS_CUSTOMER.id);
...
/*    });*/
    ...
    //    map.addImage("fiberCustomerandWirelessCustomerIcon", image, { sdf: true });

    //    map.addSource(source, {
    //        "type": "vector",
    //        "tiles": [`${store.baseApiUrl}/tiles/${store.heatmap}/addresses/{z}/{x}/{y}.mvt?customerCategoryId=1`],
    //        "tolerance": 0,
    //        promoteId,
    //    });
    ...
    //            layout: {
    //                visibility: visibility,
    //                "icon-image": "fiberCustomerandWirelessCustomerIcon",
...
export default fiberCustomerandWirelessCustomerLayer;
