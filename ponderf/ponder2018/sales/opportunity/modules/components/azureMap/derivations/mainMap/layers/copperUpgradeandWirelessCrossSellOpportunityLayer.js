...
const copperUpgradeandWirelessCrossSellOpportunityLayer = ({ instance, beforeLayerId, pointMinShapeSize }) => {
    ...
    //const iconPromises = [
    //    map.imageSprite.createFromTemplate(`copperUpgradeandWirelessCrossSellOpportunityIcon`, 'ponder-ellipse', color)
    //];
    ...
    //Promise.all(iconPromises).then(function () {
        ...
        const src = new atlas.source.VectorTileSource(source, {
            "tileSize": 256,
            "tiles": [`${store.baseApiUrl}/tiles/${store.heatmap}/addresses/{z}/{x}/{y}.mvt?customerCategoryId=4&searchedPSA=${store.searchedPSA}`]
        });
        map.sources.add(src);
        ...
        const layer = new atlas.layer.SymbolLayer(src, id, {
            ...
            minZoom,
            iconOptions: {
                image: 'copperUpgradeandWirelessCrossSellOpportunityIcon',
                allowOverlap: true,
                anchor: 'center',
        ...
        map.layers.add([layer], config.pointsLayers.COPPER_UPGRADE_AND_WIRELSS_CROSS_SELL_OPPORTUNITY.id);
