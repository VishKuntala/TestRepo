...
const wirelessCustomerandFiberCrossSellOpportunityLayer = ({ instance, beforeLayerId, pointMinShapeSize }) => {
    ...
    const {
        ...
        imgUrl,
        minShapeSize
    } = pointsLayers.WIRELESS_CUSTOMER_AND_FIBER_CROSS_SELL_OPPORTUNITY;
    ...
    //const iconPromises = [
    //    map.imageSprite.createFromTemplate(`wirelessCustomerandFiberCrossSellOpportunityIcon`, 'ponder-ellipse', color)
    //];
    ...
    //Promise.all(iconPromises).then(function () {
        ...
        const src = new atlas.source.VectorTileSource(source, {
            "tileSize": 256,
            "tiles": [`${store.baseApiUrl}/tiles/${store.heatmap}/addresses/{z}/{x}/{y}.mvt?customerCategoryId=5&searchedPSA=${store.searchedPSA}`]
        });
        map.sources.add(src);
        ...
        const layer = new atlas.layer.SymbolLayer(src, id, {
            ...
            minZoom,
            iconOptions: {
                image: 'wirelessCustomerandFiberCrossSellOpportunityIcon',
                allowOverlap: true,
                anchor: 'center',
        ...
        map.layers.add([layer], config.pointsLayers.WIRELESS_CUSTOMER_AND_FIBER_CROSS_SELL_OPPORTUNITY.id);
...
/*    });*/
    ...
    //    map.addImage("wirelessCustomerandFiberCrossSellOpportunityIcon", image, { sdf: true });

    //    map.addSource(source, {
    //        "type": "vector",
    //        "tiles": [`${store.baseApiUrl}/tiles/${store.heatmap}/addresses/{z}/{x}/{y}.mvt?customerCategoryId=5`],
    //        "tolerance": 0,
    //        promoteId,
    //    });
    ...
    //            layout: {
    //                visibility: visibility,
    //                "icon-image": "wirelessCustomerandFiberCrossSellOpportunityIcon",
...
export default wirelessCustomerandFiberCrossSellOpportunityLayer;
