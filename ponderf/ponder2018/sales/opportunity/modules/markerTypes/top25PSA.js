...
const clicked = async (instance, coordinates, psa, config) => {    
    ...
    try {
        const data = await instance.getData(
            `${store.baseApiUrl}/point/top25psas/${psa}`,
        );
        ...
        $(".psaFilterLink").on('click', () => {
            ...
            store.searchedPSA = popUpData.zip_co_psa;
            store.addPSAFilter = {
        ...
        $(".countyGeographyLink").on('click', async () => {
            console.log('.countyFilterLink clicked');
            //// remove open popup
            //$('.mapboxgl-popup').remove();
            if (instance.popup.isOpen()) {
                instance.popup.close();
        ...
        $(".zipGeographyLink").on('click', async () => {
            console.log('.zipFilterLink clicked');
            // remove open popup
            //$('.mapboxgl-popup').remove();
            if (instance.popup.isOpen()) {
                instance.popup.close();
