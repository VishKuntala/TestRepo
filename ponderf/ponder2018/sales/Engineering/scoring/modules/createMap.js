...
const createMap = async (mapOptions) => {
    resetBaseLayers();
    resetOverlayLayers();
    ...
    ).setView(mapOptions.currentMapCenter, mapOptions.currentMapZoom);
    ...
    map.on("zoomend", async function (e) {
        showHideLayers(map);
        if (mapOptions.eventHandlers && mapOptions.eventHandlers.zoomend) {
            await mapOptions.eventHandlers.zoomend(e);
        }
    });

    map.on("moveend", async function (e) {
        if (mapOptions.eventHandlers && mapOptions.eventHandlers.moveend) {
            await mapOptions.eventHandlers.moveend(e);
        }
    });
    ...
    map.on('click', async function (e) {
        ...
        if (closestLocation) {
            markerTypes[closestLocation.p.markerType].clicked(map, closestLocation, mapOptions.toolConfig);
        }
