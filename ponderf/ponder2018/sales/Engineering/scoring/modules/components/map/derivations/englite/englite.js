...
const engLiteMap = ({ adminUser, instance }) => {
    ...
    const createEngLiteLayer = async title => {
        let pane = instance.map.getPane(title);
        if (!pane) {
            pane = instance.map.createPane(title);
            pane.style.zIndex = 400;
    ...
    const createMapLayers = async () => {
        ...
        const engLiteLayer = (await createEngLiteLayer('Engineering Lite BYOC Locations')).canvasLayer;
        const storeOverlays = await createStoreLayers();
        instance.map.addLayer(engLiteLayer);
        let engLiteOverlays = {};
        engLiteOverlays["Engineering Locations"] = engLiteLayer;
        let overlays = {};
        overlays = Object.assign(storeOverlays, engLiteOverlays);
