...
const globalBusinessMap = ({ adminUser, instance }) => {
    ...
    const createGlobalBusinessLayer = async title => {
        let pane = instance.map.getPane(title);
        if (!pane) {
            pane = instance.map.createPane(title);
            pane.style.zIndex = 400;
    ...
    const createMapLayers = async () => {
        ...
        const globalBusinessLayer = (await createGlobalBusinessLayer('Global Business Locations')).canvasLayer;
        const storeOverlays = await createStoreLayers();
        instance.map.addLayer(globalBusinessLayer);
        let globalBusinessOverlays = {};
        globalBusinessOverlays["Global Business Locations"] = globalBusinessLayer;
        let overlays = {};
        overlays = Object.assign(storeOverlays, globalBusinessOverlays);
