...
async renderMap() {
    ...
    async initSubscriptions() {
                ...
                if (instance.mapData) {
                    if (window.Worker) {
                        ...
                        fetchDataWorker.onmessage = async function (e) {
                            const dataKeys = instance.store.config.dataTableDataKeys.split(',');
                            const dataTableExportOmitKeys = instance.store.config.dataTableExportOmitKeys.split(',');
                            const dataTableHeaders = instance.store.config.dataTableHeaders.split(',');
                            const headers = [];
                            for (let i = 0; i < dataKeys.length; i += 1) {
                                if (!R.includes(dataKeys[i], dataTableExportOmitKeys)) {
                                    headers.push(dataTableHeaders[i]);
                                }
                            }
                            const defaultDatakeysObject = {};
                            R.map(h => defaultDatakeysObject[h] = "", dataKeys);
                            const csv = Papa.unparse({
                                "fields": headers, "data": R.map(item => {
                                    ...
                                    return R.values(R.omit(
                                        R.concat(['etag', 'partitionKey', 'rowKey', 'timestamp'], dataTableExportOmitKeys || []),
                                        R.pick(dataKeys, itemWithNullDefaults)))
                                }, e.data)
                            });
        ...
        instance.appendSubscription(instance.stateManager.subscribe("exportSalesData",
            async d => {
                if (instance.mapData) {
                    if (window.Worker) {
                        $('body')
                        ...
                        const fetchDataWorker = new Worker("modules/components/map/derivations/fetchSalesAllDataWorker.js");
                        fetchDataWorker.postMessage({
                            postData: Object.assign(buildPayloadFromStore(instance), {
                                limit: instance.mapData.length, buildSortColumns: false,
                                sortCriteria: instance.store.config.startingSortCriteria
                        ...
                        fetchDataWorker.onmessage = async function (e) {
                            ...
                            const buffer = await workbook.xlsx.writeBuffer();
                            const blob = new Blob([buffer], { type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" });
                            saveAs(blob, "Sales_ALL_Exported_Data.xlsx");
                            $('body')
                                .toast({
                                    class: 'success',
                                    showIcon: 'arrow circle down',
