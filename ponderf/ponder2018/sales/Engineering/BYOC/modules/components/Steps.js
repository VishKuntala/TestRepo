...
class Steps extends PonderComponent {
    ...
    const response = await $.ajax({
        url: `/sales/Engineering/byoc/BYOC.aspx/${$('#frmMode').val() === 'rerun' ? 'RerunCluster' : 'SaveCluster'}`,
...
async #parseFiles() {
    ...
    reader.onload = async () => {
        try {
            ...
            else if (dataRows.length > this.appState.userPermissions.uploadLimit) {
                $('.negative.missingfile.message').removeClass('hidden').addClass('visible');
                $('.negative.missingfile.message').text(`You can only upload a maximum of ${this.appState.userPermissions.uploadLimit.toLocaleString()} locations`);
                $('#btnImport').removeClass('loading');
                $('button.next').prop('disabled', true);
                return;
            }
...
render() {
    ...
    return html`${stepsTemplate({
        ...
        uploadLimit: this.appState.userPermissions.uploadLimit.toLocaleString()
    })}`;
