...
const findClosestLocation = (zoom, latitude, longitude, locations) => {
...
const closest = (zoom, latitude, longitude, locations) => {
    const highClickPriorityLocations = locations.filter(location => location.p.clickPriority > 0);
    const lowClickPriorityLocations = locations.filter(location => location.p.clickPriority == 0);
    let foundLocation;
    if (highClickPriorityLocations.length > 0) {
        foundLocation = findClosestLocation(zoom, latitude, longitude, highClickPriorityLocations);
        if (foundLocation) {
            return foundLocation;
        }
    }
    if (lowClickPriorityLocations.length > 0) {
        foundLocation = findClosestLocation(zoom, latitude, longitude, lowClickPriorityLocations);
    }
    return foundLocation;
}
