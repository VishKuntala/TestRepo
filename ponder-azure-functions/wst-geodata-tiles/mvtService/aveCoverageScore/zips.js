...
const mvt = async (bbox, pgClient, filterParams, channel) => {
  ...
  const blobName = `ave-cov-score-heatmap-zip-${channel.toLowerCase()}-${partitionKey}.mvt`;

  await tableCacheService.initService(channel, partitionKey, null, null, null, `${filterParams.z}/${filterParams.x}/${filterParams.y}`,'zips','aveCoverageScore' );
  ...
  const results =  await pgClient.query(
    `
      ...
      geom_filter AS
      (
          SELECT id, zip, state, geom, COALESCE(${config.channelToTableColumnMappings[channel]},0)::FLOAT8 as avg_cov_score from (
            (
              SELECT gid as id, zip, state, geom from spatial.zip WHERE  ST_Intersects(geom, ST_Transform(ST_MakeEnvelope($1, $2, $3, $4, 4326), 3857))
            ) z left outer join wst.zip_channel_avg zca
            on z.id = zca.gid
          ) nf 
      )
      SELECT ST_AsMVT(q, 'zipLayer', 4096, 'geom') FROM (
      SELECT 
            id, zip, state, avg_cov_score,
