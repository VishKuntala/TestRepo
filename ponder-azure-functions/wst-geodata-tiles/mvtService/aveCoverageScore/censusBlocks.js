...
const mvt = async (bbox, pgClient, filterParams, channel) => {
  ...
  const blobName = `ave-cov-score-heatmap-census-block-${channel.toLowerCase()}-${partitionKey}.mvt`;

  await tableCacheService.initService(channel, partitionKey, null, null, null, `${filterParams.z}/${filterParams.x}/${filterParams.y}`,'censusBlocks','aveCoverageScore' );
  ...
  const results =  await pgClient.query(
        `
          ...
          (
              SELECT * from (
                SELECT cb.gid as id, cb.name, geom, COALESCE(${config.channelToTableColumnMappings[channel]},0)::FLOAT8 as avg_cov_score FROM 
                spatial.census_block cb left outer join
                wst.census_block_channel_avg cbca
                on cb.gid = cbca.gid
