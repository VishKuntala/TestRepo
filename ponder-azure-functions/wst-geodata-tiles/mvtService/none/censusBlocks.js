...
const mvt = async (bbox, pgClient, filterParams, channel) => {
  ...
  const blobName = `none-heatmap-census-block-${channel.toLowerCase()}-${partitionKey}.mvt`;

  await tableCacheService.initService(channel, partitionKey, null, null, null, `${filterParams.z}/${filterParams.x}/${filterParams.y}`,'censusBlocks','none' );
  ...
  const results =  await pgClient.query(
        `
          ...
          (
                ...
                SELECT gid as id, name, geom, 0 as avg_cov_score FROM 
                spatial.census_block 
