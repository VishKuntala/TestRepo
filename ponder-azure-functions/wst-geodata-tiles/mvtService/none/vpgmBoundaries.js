...
const mvt = async (bbox, pgClient, filterParams) => { 
    ...
    const blobName = `none-heatmap-vpgmboundaries-${partitionKey}.mvt`;

    await tableCacheService.initService(channel, partitionKey, null, null, null, `${filterParams.z}/${filterParams.x}/${filterParams.y}`,'vpgmBoundaries','none' );
