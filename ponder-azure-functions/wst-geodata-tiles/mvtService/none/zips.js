...
const mvt = async (bbox, pgClient, filterParams, channel) => {
  ...
  const blobName = `none-heatmap-zip-${channel.toLowerCase()}-${partitionKey}.mvt`;

  
  await tableCacheService.initService(channel, partitionKey, null, null, null, `${filterParams.z}/${filterParams.x}/${filterParams.y}`,'zips','none' );
  ...
  const results =  await pgClient.query(
    `
      ...
      (
        ...
        SELECT gid as id, zip, state, geom, 0 as avg_cov_score from spatial.zip WHERE  ST_Intersects(geom, ST_Transform(ST_MakeEnvelope($1, $2, $3, $4, 4326), 3857))
