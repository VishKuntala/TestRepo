...
const mvt = async (bbox, pgClient, filterParams, channel) => {
  ...
  const blobName = `location-density-heatmap-dma-${channel.toLowerCase()}-${partitionKey}.mvt`;
 
  // await tableCacheService.initService(channel, partitionKey, whereClause, null, null, `${filterParams.z}/${filterParams.x}/${filterParams.y}`,'dmas','locationDensity' );
  ...
  const results =  await pgClient.query(
        `
          ...
          (
              SELECT * from (
                SELECT py.gid as id, py.geom,density_percentage, py.dma, py.dma_name as name
                FROM spatial.dma_county py LEFT OUTER JOIN (
                SELECT (numerator*100)/denominator as density_percentage, A.dma_id from (
	             (
                 SELECT count(*) as denominator, dma_id 
	              from ${config.tableName} group by dma_id) A
	              JOIN 
                (
                  SELECT count(*) as numerator, dma_id from ${config.tableName} lus 
          ...
          SELECT 
                id, dma, name, density_percentage,
