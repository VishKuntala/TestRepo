...
const mvt = async (bbox, pgClient, filterParams, channel) => {
  ...
  const blobName = `fiber-penetration-heatmap-county-${channel.toLowerCase()}-${partitionKey}.mvt`;
  ...
  const results =  await pgClient.query(
        `
          ...
          (
              SELECT * from (
                SELECT py.gid as id, py.geom,fiber_penetration, py.name, py.cnty_fips
                FROM spatial.county py LEFT OUTER JOIN (
                 SELECT lus.county_gid, sum(lus.fiber_customers)*100/sum(lus.fiber_opportunity) as fiber_penetration 
          ...
          SELECT 
                id, name, cnty_fips, fiber_penetration,
