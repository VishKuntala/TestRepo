...
const mvt = async (bbox, pgClient, filterParams, channel) => {
  ...
  const blobName = `fiber-penetration-heatmap-dma-${channel.toLowerCase()}-${partitionKey}.mvt`;
  ...
  const results =  await pgClient.query(
        `
          ...
          (
              SELECT * from (
                SELECT py.gid as id, py.geom,fiber_penetration, py.dma, py.dma_name as name
                FROM spatial.dma_county py LEFT OUTER JOIN (
                 SELECT lus.dma_gid, sum(lus.fiber_customers)*100/sum(lus.fiber_opportunity) as fiber_penetration 
