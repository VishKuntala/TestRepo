...
const mvt = async (bbox, pgClient, filterParams, channel) => {
  ...
  const blobName = `fiber-penetration-heatmap-census-block-${channel.toLowerCase()}-${partitionKey}.mvt`;
  ...
  const results =  await pgClient.query(
        `
          ...
          (
              SELECT * from (
                SELECT py.gid as id, py.geom,fiber_penetration, py.name
                FROM spatial.census_block py LEFT OUTER JOIN (
                 SELECT lus.census_block_gid, sum(lus.fiber_customers)*100/sum(lus.fiber_opportunity) as fiber_penetration 
