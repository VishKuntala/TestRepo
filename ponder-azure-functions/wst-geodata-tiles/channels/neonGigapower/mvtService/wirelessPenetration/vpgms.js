...
const mvt = async (bbox, pgClient, filterParams, channel) => {
  ...
  const blobName = `wireless-penetration-heatmap-vpgm-${channel.toLowerCase()}-${partitionKey}.mvt`;
  ...
  const results =  await pgClient.query(
        `
          ...
          (
              SELECT * from (
                SELECT py.gid as id, py.geom,wireless_penetration, py.vpgm_markt as name
                FROM spatial.vpgm_market py LEFT OUTER JOIN (
                 SELECT lus.sales_market_gid, sum(lus.wireless_customers)*100/sum(lus.fiber_available) as wireless_penetration 
