...
const mvt = async (bbox, pgClient, filterParams, channel) => {
  ...
  const blobName = `wireless-penetration-heatmap-zip-${channel.toLowerCase()}-${partitionKey}.mvt`;
  ...
  const results =  await pgClient.query(
        `
          ...
          (
              SELECT * from (
                SELECT py.gid as id, py.geom,wireless_penetration, py.zip, py.state
                FROM spatial.zip py LEFT OUTER JOIN (
                 SELECT lus.zip, sum(lus.wireless_customers)*100/sum(lus.fiber_available) as wireless_penetration 
