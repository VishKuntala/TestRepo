...
const mvt = async (bbox, pgClient, filterParams, channel) => {
  ...
  const blobName = `wireless-penetration-heatmap-dma-${channel.toLowerCase()}-${partitionKey}.mvt`;
  ...
  const results =  await pgClient.query(
        `
          ...
          (
              SELECT * from (
                SELECT py.gid as id, py.geom,wireless_penetration, py.dma, py.dma_name as name
                FROM spatial.dma_county py LEFT OUTER JOIN (
                 SELECT lus.dma_gid, sum(lus.wireless_customers)*100/sum(lus.fiber_available) as wireless_penetration 
	              from ${config.tableName} lus 
                  inner join wst.neon_gigapower_fiber_addresses ts on lus.zip_co_psa = ts.zip_co_psa 
                  WHERE 1=1 ${whereClause}
                  group by lus.dma_gid) A
	              on py.gid = A.dma_gid        
          ...
          SELECT 
                id, name, dma, wireless_penetration,
