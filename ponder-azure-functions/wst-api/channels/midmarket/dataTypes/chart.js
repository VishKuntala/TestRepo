...
module.exports = async (pgClient, postParams, getParams, entityType) => {
    const { attribute } = postParams;
    const whereClause = sqlBuilder.whereClause(postParams);

    const sql = `SELECT count(*) as count, ${attribute} from ${config.tableName}
    WHERE COVERAGE_SCORE IS NOT NULL AND COVERAGE_SCORE_RANK != 0 AND ${whereClause} 
    GROUP BY ${attribute}
    `;
