...
const whereClause = (params,userAttributes) =>
{
    ...
    //if (channelCode && channelCode == "midmarketFiber" )
    //{       
    //    sql = sql + ` AND (owning_sales_center_id like 'SB%' or owning_sales_center_id like 'SELECT%') and (owning_module not like '%-RD%') `;
    //}
    ...
    if (salesMarket)
    {
        if (authorizedSalesMarketValues) {
            ...
            sql = sql + " AND sales_market_id IN (" + allowedValues + ")";
        } else {
            sql = sql + " AND sales_market_id = " + salesMarket + "";
        }
    } else {
        sql = sql + " AND sales_market_id IN (" + authorizedSalesMarketValues + ")";
    }
    if (archetype)
    {
