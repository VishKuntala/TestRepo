...
const get = async (pgClient, params) => {
  ...
  const whereClause = () => {
    ...
    if (marketTypeVariable === "MARKET") {
      whereCondition = `${whereCondition} AND (SALES_MARKET1 = '${marketVariable}' OR SALES_MARKET2 = '${marketVariable}'  OR SALES_MARKET3 = '${marketVariable}')`;
    } else if (marketTypeVariable === "REGION") {
      whereCondition = `${whereCondition} AND (SALES_REGION1 = '${marketVariable}' OR SALES_REGION2 = '${marketVariable}')`;
    }
  ...
  totalPagesResult = await pgClient.query(
    `SELECT CEIL(count(*)/(${
      limit || defaultLimit
    }*1.00)) last_page FROM SOT.sales_opp_dma_prod 
  ...
  result = await pgClient.query(
    ...
    DMA, DMA_NAME, ROUND(TOTALAREA,0) AS TOTALAREA, TO_CHAR(TOTALPOPS + 0, '999,999,999,999') AS TOTALPOPS, 
    ...
    TO_CHAR(FIBER_CUSTOMERS, '999,999,999,999') AS FIBER_CUSTOMERS, FIBER_PERCENTAGE::FLOAT AS FIBER_PERCENTAGE, OVERALL_CAT, TO_CHAR(FIBER_OPPORTUNITY, 
    '999,999,999,999') AS FIBER_OPPORTUNITY, NUMBER_OF_PSAS, WIRELESS_CATEGORY, 
