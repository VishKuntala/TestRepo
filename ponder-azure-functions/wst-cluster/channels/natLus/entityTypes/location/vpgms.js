  ...
  return pgClient.query(
    `
      ...
      (
            ...
            py.vpgm_markt as name, numerator as numberofmatchedlocations
            FROM spatial.vpgm_market py INNER JOIN 
            (
              SELECT count(*) as numerator, sales_market_id from ${config.tableName}
              WHERE 1=1  ${whereClause}
              group by sales_market_id
            ) B
              on py.gid = B.sales_market_id WHERE ST_Intersects(py.geom, ST_Transform(ST_MakeEnvelope($1, $2, $3, $4, 4326), 3857)) 
      )
