...
const mvt = async (bbox, pgClient, filterParams) => {
  ...
  const containerName = `wst-data-cache-${channel}-county-clusters`;
  // Create a unique name for the blob
  const blobName = `${channel}-county-cluster-${partitionKey}.mvt`;
    ...
    const results = await pgClient.query(
        `
          ...
          (
            ...
            (
            SELECT lus.county_id ,count(distinct lus.mikey) as numberofmatchedlocations
            FROM ${config.tableName} lus
            inner join wst.bcls_tenant_summary ts on lus.mikey = ts.mikey
            WHERE 1=1 ${whereClause}
            group by lus.county_id) as B
            ON A.id = B.county_id
          )
