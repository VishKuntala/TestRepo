...
const mvt = async (bbox, pgClient, filterParams) => {
  ...
  const containerName = `wst-data-cache-${channel}-dma-clusters`;
  // Create a unique name for the blob
  const blobName = `${channel}-dma-cluster-${partitionKey}.mvt`;
      ...
      const results = await pgClient.query(
        `
          ...
          (
            ...
            (
            SELECT lus.dma_id,count(distinct lus.mikey) as numberofmatchedlocations
            FROM ${config.tableName} lus
            inner join wst.bcls_tenant_summary ts on lus.mikey = ts.mikey
            WHERE 1=1 ${whereClause}
            group by lus.dma_id) as B
            ON A.id = B.dma_id            
          )
