...
const mvt = async (bbox, pgClient, filterParams) => {
  ...
  const containerName = `wst-data-cache-${channel.toLowerCase()}-county-clusters`;
  // Create a unique name for the blob
  const blobName = `${channel.toLowerCase()}-county-cluster-${partitionKey}.mvt`;

  // await tableCacheService.initService(channel, partitionKey, whereClause, null, null, `${filterParams.z}/${filterParams.x}/${filterParams.y}`,'counties' );
