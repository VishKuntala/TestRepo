...
const { entityTypes } = require('./channels/midmarket');
...
module.exports = async function (context, req) {
    ...
    try {
        ...
        const results = await channels[channel]["entityTypes"][req.params.entityType][req.params.geotype].mvt(bbox, pool, Object.assign(req.query, { attuid, channel, accessControlId, x: req.params.x, y: req.params.y, z: req.params.z }));
        if (!results) {
        ...
        if (tile?.st_asmvt.length === 0) {context.res = {status: 204};}
        else {context.res = { 'Content-Type': 'application/x-protobuf', body: tile.st_asmvt};} */
