...
const mvt = async (bbox, pgClient, filterParams) => {  
  ...
  // const blobName = `fiber-penetration-heatmap-zip-${partitionKey}.mvt`;
  ...
  const results = await pgClient.query(
    ...
    (
        SELECT * from (                
                SELECT dim.gid as id, dim.zip, dim.name, dim.geom,
                fct.copper_upgrade, fct.totalarea, fct.totalpops, fct.pop_density, fct.wireless_score, fct.fiber_score,
                COALESCE(fct.fiber_available, 0) as fiber_available, fct.fiber_customers, COALESCE(fct.fiber_percentage, 0)::float as fiber_percentage, fct.overall_cat, fct.state,
                ...
                FROM spatial.zip as dim
                LEFT OUTER JOIN sot.sales_opp_zip_prod fct on dim.zip = fct.zip
    ...
    SELECT ST_AsMVT(q, 'zipLayer', 4096, 'geom') FROM (    
            ...
            fiber_available, fiber_customers, fiber_percentage::float, overall_cat, state,
