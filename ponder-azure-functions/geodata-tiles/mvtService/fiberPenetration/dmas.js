...
const mvt = async (bbox, pgClient, filterParams) => {
  ...
  const whereClauseForFilters = () => {
    ...
    if (marketType == "REGION") {
      whereClause = `${whereClause} and UPPER(fct.sales_region1) = UPPER('${market}') OR UPPER(fct.sales_region2) = UPPER('${market}')`;
    }
    if (marketType == "MARKET") {
      whereClause = `${whereClause} and UPPER(fct.sales_market1) = UPPER('${market}') OR UPPER(fct.sales_market2) = UPPER('${market}') OR UPPER(fct.sales_market3) = UPPER('${market}')`;
    }
    return whereClause;
  };
  ...
  // const containerName = `fot-data-cache-fiber-penetration-heatmap`;
  // // Create a unique name for the blob
  // const blobName = `fiber-penetration-heatmap-dma-${partitionKey}.mvt`;
  ...
  const results = await pgClient.query(
    ...
    (
        SELECT * from (
            SELECT dim.gid as id, dim.dma, dim.dma_name as name, dim.geom,
            ...
            FROM spatial.dma_county as dim
            LEFT OUTER JOIN sot.sales_opp_dma_prod fct on dim.dma = fct.dma::varchar(3) 
